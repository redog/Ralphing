# nidhinjs/prompt-master — Claude Skill for Writing Accurate Prompts (GitHub)

Source: https://github.com/nidhinjs/prompt-master
Category: awesome-prompts-tools
...
| 5 | Emotional task description | "it's totally broken, fix everything" | "Throws uncaught TypeError on line 43 when useris null" | 
...
| 23 | No file path for IDE AI | "update the login function" | "Update handleLogin()insrc/pages/Login.tsxonly" | 
...
| # | Pattern | Before | After | 
|---|---|---|---|
| 31 | No starting state | "build me a REST API" | "Empty Node.js project, Express installed, src/app.jsexists" | 
| 32 | No target state | "add authentication" | " /src/middleware/auth.jswith JWT verify.POST /loginandPOST /registerin/src/routes/auth.js" | 
| 33 | Silent agent | No progress output | "After each step output: ✅ [what was completed]" | 
| 34 | Unlocked filesystem | No file restrictions | "Only edit files inside src/. Do not touchpackage.json,.env, or any config file." | 
...
MIT: See [LICENSE](https://github.com/nidhinjs/prompt-master/blob/main/LICENSE) for details.
